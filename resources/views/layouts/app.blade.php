...
<body
    class="bg-[#f8fafc] dark:bg-slate-900 text-slate-800 dark:text-slate-200 antialiased selection:bg-indigo-100 selection:text-indigo-700 dark:selection:bg-indigo-900/50 dark:selection:text-indigo-300">
    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header
                class="flex items-center justify-between h-16 px-6 lg:px-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-800 sticky top-0 z-10 shadow-sm transition-colors print:hidden">
                ...
                <div class="w-1/3 flex justify-center">
                    <h1
                        class="text-2xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-violet-600 dark:from-indigo-400 dark:to-violet-400 drop-shadow-sm">
                        TOKO HASAN
                    </h1>
                </div>
                ...
                <div class="w-1/3 flex justify-end items-center space-x-5">
                    ...
                    <button id="theme-toggle" type="button"
                        class="text-slate-400 hover:text-indigo-600 dark:hover:text-amber-400 transition-colors p-2 rounded-lg"
                    ...
                    <div class="relative">
                        <button id="notif-btn" type="button"
                            class="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors relative focus:outline-none">
                            @if(isset($notifications) && $notifications->count() > 0)
                                <span
                                    class="absolute top-0 right-0 w-2 h-2 bg-rose-500 rounded-full border-2 border-white dark:border-slate-900 animate-pulse"></span>
                            @endif
                        ...
                        <!-- Dropdown Panel -->
                        <div id="notif-menu"
                            class="hidden absolute right-0 mt-3 w-80 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden z-50 transform origin-top-right transition-all">
                            <div
                                class="p-4 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/50">
                                <h3
                                    class="text-sm font-bold text-slate-800 dark:text-white flex justify-between items-center">
                                    Notifikasi
                                    @if(isset($notifications) && $notifications->count() > 0)
                                        <span
                                            class="bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 px-2 py-0.5 rounded-md text-xs">{{ $notifications->count() }}
                                            Baru</span>
                                    @endif
                                </h3>
                            </div>
                            <div class="max-h-96 overflow-y-auto">
                                @if(isset($notifications) && $notifications->count() > 0)
                                    @foreach($notifications as $notif)
                                        <div
                                            class="p-4 border-b border-slate-50 dark:border-slate-700/30 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors cursor-default">
                                            <div class="flex gap-3">
                                                <div class="flex-shrink-0 mt-0.5">
                                                    @if($notif['type'] == 'danger')
                                                        <div
                                                            class="w-8 h-8 rounded-full bg-rose-100 dark:bg-rose-900/30 flex items-center justify-center text-rose-500 dark:text-rose-400">
                                                            @if($notif['icon'] == 'clock')
                                                    ...
                                                    @else
                                                        <div
                                                            class="w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center text-amber-500 dark:text-amber-400">
                                                            @if($notif['icon'] == 'cube')
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                                    viewBox="0 0 24 24">
                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                                                        {!! $notif['message'] !!}</p>
                                                    <p
                                                        class="text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 font-medium">
                                                        {{ $notif['time'] }}</p>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                @else
                                    <div class="px-4 py-8 text-center flex flex-col items-center">
                                        <svg class="w-12 h-12 text-slate-200 dark:text-slate-700 mb-3" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
                                        <p class="text-sm font-bold text-slate-400 dark:text-slate-500">Tidak ada notifikasi
                                            baru.</p>
                                    </div>
                                @endif
                            </div>
                            <div
                                class="px-4 py-3 bg-slate-50/50 dark:bg-slate-800/80 border-t border-slate-100 dark:border-slate-700/50 text-center">
                                <a href="#"
                                    onclick="document.getElementById('notif-menu').classList.add('hidden'); event.preventDefault();"
                                    class="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300">Tutup
                                    Notifikasi</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 pl-5 border-l border-slate-200 dark:border-slate-700">
                        <div class="hidden md:block text-right">
                            <p class="text-sm font-semibold text-slate-700 dark:text-slate-200 leading-tight">Admin
                                Hasan</p>
                            <p class="text-[11px] text-slate-500 dark:text-slate-400 font-medium tracking-wide">
                                ADMINISTRATOR</p>
                        </div>
                        <div
                            class="h-9 w-9 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold shadow-md cursor-pointer hover:shadow-lg hover:scale-105 transition-all ring-2 ring-white dark:ring-slate-800">
                            A
                        </div>
                    </div>
            ...
            <main class="flex-1 overflow-y-auto p-6 lg:p-8 print:overflow-visible print:p-0">
                ...
                <div class="max-w-6xl mx-auto">
                    ...
                    @if(session('success'))
                        <div class="mb-6 p-4 rounded-2xl bg-emerald-500/10 dark:bg-emerald-500/20 border border-emerald-500/20 text-emerald-700 dark:text-emerald-300 flex items-center gap-3 text-sm font-medium shadow-sm transition-all duration-300">
